...
class Absensi extends CI_Controller {
	...
	public function masuk()
	{
		...
		$cabang 	= $this->db->query("SELECT * FROM tb_cabang where id='$idCabang'")->result_array();
		$url		= 'https://maps.google.com/maps?&z=15&mrt=yp&t=k&q=' . $latitude . '+' . $longitude;
		foreach ($cabang as $key) {
        $cabangLatitude = $key['latitude'];
        $cabangLongitude = $key['longitude'];
        $jarak = $this->haversineDistance($latitude, $longitude, $cabangLatitude, $cabangLongitude);
        if ($jarak <= 100) {
			...
			$this->session->set_flashdata('pesanError', 'Anda diluar area absen');
			redirect('admin/absensi');
		}
		}
